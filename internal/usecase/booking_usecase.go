...
func (b *bookingUsecase) SelectSeat(ctx context.Context, userID, showID, seatNo string) error {
	...
	if err != nil {
		b.publishErrorLog("Lock fail", fmt.Sprintf("User %s failed to acquire lock for seat %s due to system error: %v", userID, seatNo, err))
		return err
	}
	if !acquired {
		b.publishErrorLog("Lock fail", fmt.Sprintf("User %s tried to lock seat %s but it is already locked by another process", userID, seatNo))
	...
	err = b.seatRepo.UpdateSeatStatus(ctx, showID, seatNo, domain.SeatLocked, userID) 
	if err != nil {
...
func (b *bookingUsecase) ConfirmPayment(ctx context.Context, userID, showID, seatNo string) error {
	// อัปเดตสถานะเป็น BOOKED ลงฐานข้อมูลหลัก
	err := b.seatRepo.UpdateSeatStatus(ctx, showID, seatNo, domain.SeatBooked, userID) 
	if err != nil {
		return err
	}

	// คลาย Distributed Lock ใน Redis
...
func (b *bookingUsecase) handleBookingTimeout(showID, seatNo, userID string) {
	...
	// (หมายเหตุ: ควรมีฟังก์ชันตรวจสอบสถานะปัจจุบันของที่นั่งก่อนอัปเดตคืน)
	
	// จำลองกรณีหมดเวลาชำระเงิน: ปล่อยล็อกและคืนสถานะเป็น AVAILABLE
